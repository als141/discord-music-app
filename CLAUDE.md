...
### 2026-09-21: 浸透計画 W1〜W2 の実装（commit f947219、棚 / Now Playing / チャットハーネス）
- **棚（shared_links）**: `services/shared_links.py` が `on_message` の先頭で YouTube リンク（youtube.com/watch, youtu.be, shorts, music.youtube.com）を `shared_links` テーブルに保存（URL・動画ID・投稿者・時刻・チャンネルのみ。**本文は保存しない、絶対に send しない**）。bot 専用チャンネルと bot 投稿は対象外（`IRINA_SHELF_ALLOW_BOTS=1` はローカルテスト用）。タイトル/アーティスト/サムネは `ytmusicapi.get_song` で後追い解決（起動 20 秒後と 10 分ごと、1 回 40 件、3 回失敗で諦め）。API: `GET /shared-tracks/{guild_id}?limit=200&channel_id=`（web のみで動く。同じ動画は最新 1 件、未解決のサムネは `i.ytimg.com/vi/{id}/hqdefault.jpg`）。**過去分**: `scripts/backfill_shared_links.py <guild_id> --days 90` を Pi の backend/ で実行（2026-09-21 にドデカ 90 日分 66 件＝一般 48 + 曲置き場 18 を取り込み済み）
- **Now Playing の可視化（`services/now_playing_status.py`）**: `MusicPlayer` の再生開始/キュー終了/シャットダウンで `register_now_playing_hook` 経由に通知 → ①bot Presence を「曲名 — アーティスト を再生中」（Listening）、何も無ければ `CustomActivity('バージョン1.0.0')` ②VC チャンネルステータス `♪ 曲名 — アーティスト`（`client.http.edit_voice_channel_status`。discord.py 2.7.1 の `VoiceChannel.edit` には `status` が無いので HTTP 層を直接呼ぶ）。**チャンネルごと 30 秒デバウンス**（即 PUT は 429/500 になる実運用報告あり）、`Forbidden` ならそのサーバーで自己無効化、切断/終了で `None`、lifespan shutdown で `clear_all()`。ログ `[now-playing] VC status set/cleared`
- **チャットのハーネス化（`services/irina_chat.py` 全面改訂）**: ①会話履歴は **xAI 側に保存**（`store_messages=True` + `previous_response_id`）。チャンネルごとの最後の response_id を `chat_sessions` に持つので再起動しても続く。チェーンの継続に失敗したら 1 回だけ新チェーンで再試行 ②**圧縮**: 40 ターン or prompt 15 万トークンで会話全体を要約（`_compact`）→ `chat_sessions.summary` に保存し新チェーン（要約は次の system prompt に入る）③**長期メモ**: `remember` / `forget` ツール → `chat_memory`（サーバーごと 40 件まで system prompt に注入）④**汎用 HTTP ツール `http_request`**（`services/irina_tools.py`）: `service="irina"`（`IRINA_API_BASE` 既定 http://127.0.0.1:8080＝web）と `service="discord"`（REST v10、bot トークン）を curl のように 1 ツールで。破壊的 Discord 操作（サーバー/チャンネル削除、BAN/キック、ロール・権限、Webhook、アプリ設定、bulk-delete、メンバー PATCH）は既定拒否（`IRINA_TOOL_ALLOW_DANGEROUS=1` で解除）、書き込み 20 回/分、本文 6000 文字で切る。ログ `[tool] irina POST /add-url/… → 200 (by 名前(id))` ⑤ツール往復は 8 回まで、全体 240 秒 ⑥新チェーン開始時は直近 10 件のチャンネルの流れを参考として渡す ⑦キャラ設定ファイルが変わるとチェーンを切り替える
- **対象チャンネル**: 既定 `1232618506303045702`（テストサーバー #riona）と `1156255909446680676`（ドデカ bot 専用）。`IRINA_CHAT_CHANNEL_IDS` で上書き
- **検証（ローカル分割構成・テストbot）**: 雑談 → VC 一覧（tool）→「一般 VC に入って夜に駆けるをかけて」（`GET /search` → `POST /join-voice-channel` → `POST /add-url` を自走）→ remember →「頼んだことを順番に」（チェーン継続で全部正解）→ #一般 にリンク投稿で棚に収集・タイトル解決。ローカルの再生自体は古い cookie で 403（Pi では問題なし）
- **名称**: UI 上の呼び名は「棚」ではなく **「曲置き場」**（ユーザー指定。コード上の識別子 shelf/shared_links はそのまま）。**曲置き場チャンネル（1426606582606860428）は全期間**取り込み済み（2025-10-11〜、120 件。`--days 0 --channel-id` オプション）。一般は直近 90 日（48 件）
- **frontend（commit a4b18b18 → マージ c87123a9）**: `components/home/ShelfScreen.tsx`（曲置き場タブ。`HomeScreen` のタブが ホーム/棚/ライブラリ。カード全体タップ＝`addToQueue`、チャンネルのチップで絞り込み、30 秒キャッシュ、未解決はサムネ `i.ytimg.com` + 「YouTube 動画」）、`components/PreviewApp.tsx`（**ログイン前プレビュー**。`MainApp` の unauthenticated 分岐が `IntroPage` → `PreviewApp`。ギルドは `useGuildStore.activeServerId` → `NEXT_PUBLIC_DEFAULT_GUILD_ID` → ドデカ。`/player-state` を 30 秒ポーリング、曲タップ/再生で初めて `signIn('discord')`）、`utils/api.ts` に `api.getSharedTracks()`。`IntroPage.tsx` は未参照で残置
- **ローカルディスク**: 2026-09-21 に `/` が 100%（空き 336MB）でビルド/スクショが落ちた。`~/.npm/_npx`（3G）`~/.cache/uv`（3.4G）`~/.bun/install/cache`（3.1G）、未使用の Playwright Chromium、`backend/music/*`、終わった worktree を消して 5.6GB 確保。**ビルド前に `df -h /` を見る**
- **xai-sdk 1.19 対応版（ハーネス v3、2026-09-21 深夜）**: ①**ストリーミング** `chat.stream()` → `_Renderer` が返信 1 通を 1.5 秒ごとに編集して育て、最後に確定（`IRINA_CHAT_STREAM=0` で一括）。**本文は `Response.content`（SDK 側の積み上げ）を使う**: chunk の差分を自前連結すると code_execution 等のサーバー側ツールを挟んだときに本文が二重になる（実測） ②**サーバー側ツール追加**: `code_execution()`（計算・集計。`IRINA_CHAT_CODE_EXEC=0` で無効）、`image_generation()`（画像生成 ≈ $0.06/枚。`Response.image_outputs[i].image`（bytes）を `discord.File` で添付。返信送信後に生成されるので画像は続きのメッセージになる。サーバーごと 1 日 20 枚 `IRINA_CHAT_IMAGE_DAILY_LIMIT`、日付は 5 時区切り） ③**`Response.tool_calls` には完了済みのサーバー側ツール（image_generation 等）も混ざる** → `get_tool_call_type(tc) == "client_side_tool"` かつ名前が http_request/remember/forget のものだけ実行（これを忘れると「unknown tool」を返してモデルが「画像生成が死んでる」と言い出す。実測） ④`XAI_REASONING_EFFORT`（none/low/medium/high/xhigh、**既定 medium**＝ユーザー指定 2026-09-21。low なら雑談 3〜8 秒、medium は数秒増）、`XAI_SERVICE_TIER`（default/priority。priority は優先処理でコスト増）、`user="discord:<user_id>"` ⑤ログに `cost=$… (today $…) reasoning=… cached=…`（`Response.cost_usd`、`usage.reasoning_tokens`、`cached_prompt_text_tokens`。プロンプトキャッシュが効いて 2 回目以降は cached が大きい） ⑥失敗時は途中まで表示した返信に「（途中で失敗した）」を付ける。検証（テストbot）: 2^64 の計算 → 正解、白猫の画像 → 添付 1 枚、直前 2 件の順序 → 正解、返信は edited=True（ストリーミング）
- **添付対応（`_attachment_parts`）**: 画像（8MB 以下は base64 の data URL で直接渡す。それ以上は Discord の署名 URL）/ PDF・docx・xlsx・pptx・テキスト・コード（60k 文字まで）は `xai_sdk.chat.file(data=bytes, filename, mime_type)` でそのまま渡す（grok-4.6 が読める。txt/PDF/CSV で実測）/ 音声・ボイスメッセージ（ogg/mp3/m4a/wav…）は OpenAI `gpt-4o-mini-transcribe`（失敗時 whisper-1）で文字起こしして本文に添える（`OPENAI_API_KEY` が無ければ「文字起こしできなかった」）/ 動画・その他は名前だけ。上限 5 個・20MB。**`fetch_url` ツール**（公開 URL の本文 8000 文字、HTML はタグ除去、内部アドレス拒否）で「このリンク見て」に対応。検証（テストbot）: 画像→ジャケット判定、memo.txt→合言葉、PDF→コード読取、TTS で作った mp3→内容把握、example.com→タイトル。全 OK
- **参照画像つき生成・追加編集**: ユーザーが画像を添付して「これを参考に〜作って」→ 添付が `image()` で会話に入るので `image_generation()` がそれを参照して生成する（実測: 夜に駆けるのジャケット + 「同じ雰囲気で黒猫が夜の街」→ ピンク基調・フェンス・構図を引き継いだ画像、20 秒、$0.067）。続けて「さっきの猫を白猫に」→ 同じチェーン内で編集（14 秒、$0.074。`ImageGenerationOutput.image_uuid` を xAI 側が参照）。直接 API `client.image.sample(prompt, model="grok-imagine-image-2.0", image_url=data_url)` でも可（async では `res.image` が awaitable）。**注意: `AsyncClient` はイベントループの中で作る**（外で作ると grpc が別ループに縛られて "attached to a different loop" になる。本番は `_get_client()` が遅延生成なので OK）
- **応答条件と文脈量（2026-09-23）**: bot 専用チャンネル（`IRINA_CHAT_CHANNEL_IDS`）では全発言に返す。**それ以外のテキストチャンネルでは @イリーナ（`me in message.mentions`。@everyone/@here は含めない）か、イリーナの発言への返信（`message.reference` を fetch して author が bot）のときだけ返す**（`_is_addressed`）。新チェーン開始時の直近文脈は **60 件**（`IRINA_CHAT_CONTEXT_MESSAGES`、以前 10）、継続時は `chat_sessions.last_message_id` 以降の発言を最大 **80 件**（`IRINA_CHAT_GAP_MESSAGES`）毎回「前回の返事のあとの流れ」として渡す＝呼ばれたときだけ返すチャンネルでも会話を追える。DB 列は `init_db` の `ALTER TABLE` で自動追加。検証（テストbot・#一般）: メンション無し 2 件→無反応、@→間の 2 発言を把握、返信→応答
- **system prompt の「束」（2026-09-22、`services/irina_prompt.py`）**: Pi の **`/home/als0028/irina/`**（`IRINA_PROMPT_DIR`）を次の順で連結して system prompt の先頭にする（内容は加工しない。CSV だけ箇条書きに変換）: `persona.md`（旧 `~/irina_persona.txt` をコピー。無ければ旧ファイルを読む）→ `instructions.md`（ユーザーが書く指示。初期版は語録の使い方と X トレンド参照）→ `style.md`（任意、話し方の実例）→ `knowledge/*.md|txt|csv`（`<knowledge name="…">` で囲む。語録 CSV `淫夢語録_全表.csv` 276 行は `IRINA_KNOWLEDGE_CSV_COLUMNS=語録,使用方法,カテゴリ` で 9.2k トークン。全列だと 16.3k）→ `knowledge/_auto/x_trends.md`（自動生成）。全ファイルの mtime/size のハッシュがチェーンのキー＝どれか保存した次の発言から新チェーン。ログ `[irina-prompt] bundle loaded …`（ファイル名とサイズのみ）
- **X トレンドメモ（`services/irina_trends.py`）**: grok + x_search/web_search で「今日の X の言い回し・語録の現役/廃れ・派生・今日のネタ」を 1200 字以内にまとめ `knowledge/_auto/x_trends.md` へ。起動 30 秒後（前回から 20 時間超なら）と毎日 `IRINA_TRENDS_HOUR`（既定 18 時 JST。負の値で自動更新オフ）。手動は `refresh_x_trends` ツール（「トレンド更新して」）。**1 回 ≈ $0.35〜0.5**（x_search が多い）→ 月 $10〜15。ログ `[irina-trends] refreshed`。メモは各ミームの「構文テンプレ（〔穴埋め〕）＋実際の投稿例 2 つ＋派生」を含む（雑談 bot が真似できる形。例: INMU KING → 「〔対象〕をINMU KING踊らせる」「INMU KINGの刑」）。**その日の新ネタ（例: INMU KING）は instructions.md の指示で、見慣れない語が出たら x_search で今日の使われ方を見てから構文を真似る**（1 回 ≈ $0.16、+14 秒）。Pi 上でメモを手動生成するには `cd backend && .venv/bin/python -c "…irina_trends.refresh(AsyncClient(...), 'grok-4.7')"`（CLAUDE.md 2026-09-22 の履歴参照）
- **system prompt の方針（ユーザー指定 2026-09-21）**: **`~/irina_persona.txt` の内容 + 【参考情報】（場所/ID、使えるツールと API 一覧、添付の扱い、Discord の Markdown/embed の書き方）+ メモ + 要約だけ**。応答の長さ・話し方・禁止事項などの「ルール」は一切入れない（以前の `RULES`＝「1〜4 文・600 文字程度」は撤去）。`max_tokens` / `temperature` も渡さずモデル既定。`/ ! ? ;` 始まりの無視も撤去。残しているのはコスト/乱用のガードだけ: チャンネル 12 発言/分、画像 20 枚/日、ツール往復 8 回、`http_request` の破壊的 Discord 操作の既定拒否（`IRINA_TOOL_ALLOW_DANGEROUS=1` で解除）、書き込み 20 回/分
- **落とし穴（2026-09-21 05:16 実害）**: 会話履歴を xAI 側のチェーンで持つため、**system prompt を変えても既存チャンネルのチェーンには古い system prompt が残り続ける**（RULES 撤去後も「1〜4 文」のまま短文だった。ログは turn=4 で effort=medium なのに短い）。対策: `_harness_prompt_version()`（参考情報の文面 + 組み立てコードの sha1）を session key に含め、変わったら自動で新チェーン。手動で切りたいときは `/clear_chat`、または Pi で `update chat_sessions set last_response_id=NULL, turn_count=0`
- **2026-09-22: grok-4.7 に切替 + 明示的な画像生成ツール**: `XAI_MODEL` 既定 **grok-4.7**（同日リリース。`/v1/language-models` で実在確認、text+image 入力、価格は 4.6 と同じ）。画像について実測した事実: ①会話中の `image_generation`（サーバー側ツール）は **JPEG 固定（1408²、$0.059/枚 ≒ grok-imagine-image-2.0 の価格）でモデル指定・透過は不可** ②`client.image.sample()` も JPEG（RGB）で、透過やアスペクト比以外の指定は無い → **`services/irina_images.py` + クライアント側ツール `generate_image`** を追加: モデル指定（既定 `XAI_IMAGE_MODEL`=grok-imagine-image-2.0、`grok-imagine-image-quality`=pro も可）、PNG 出力、`transparent=true` で背景透過（**rembg[cpu] を依存に追加済み**＝AI 背景除去 u2netp。Pi 4 実測: 初回 import+モデル読込 264 秒（コールド）・除去 3.8 秒/枚・メモリ +470MB → `on_ready` で別スレッド先読み `irina_images.warmup()`、ログ `[irina-images] rembg ready in Ns`（本番実測 2026-09-22: 再起動から 232 秒で ready、Pi のメモリ使用 1.1GB → 1.45GB/3.8GB。デプロイ直後の約 4 分は透過依頼がクロマキーになる）。`IRINA_REMBG_WARMUP=0` で先読み無効、`IRINA_REMBG_MODEL` でモデル変更。失敗時は四隅の色によるクロマキーにフォールバック。透過時はプロンプトに「単色の白背景」を足す）、アスペクト比、`use_attached_images` で添付を参照画像に。生成物は `ctx["files"]` に集めて返信に添付、1 日の枚数上限は `image_generation` と共有。実測: 透過 PNG 11 秒 $0.04、JPG 16:9 20 秒 $0.04、参照＋quality 7 秒 $0.06。画像モデル一覧は `GET /v1/image-generation-models`（grok-imagine-image $0.02 / -2.0 $0.06 / -quality $0.05 の目安）
- **ハーネスの設計メモ（ユーザー説明用）**: Claude Code / Codex 的な要素のうち入れたのは「サーバー側の会話状態（chain）」「自動要約による圧縮」「長期メモ」「単一の汎用ツール + 安全弁」「ツールループの上限」。入れていないのは「計画/サブタスク」「ファイル編集」「コード実行（xAI の `code_execution` は未有効化）」
...
- **モデル/ツール**: `XAI_MODEL`（既定 **grok-4.6**。`GET https://api.x.ai/v1/models` で実在確認済み、画像入力可）。`xai_sdk` 1.7.0 の `AsyncClient` + サーバーサイドツール `web_search(user_location_country="JP")` / `x_search()`、`tool_choice="auto"`。system prompt = **キャラ設定（優先順: ①ファイル `IRINA_PERSONA_FILE`＝既定 `/home/als0028/irina_persona.txt`（Pi、リポ外）②環境変数 `IRINA_CHAT_PERSONA` ③`irina_chat.DEFAULT_PERSONA`＝大人の魔王・毒舌・ネットスラング版）を一切加工せずそのまま**。①は毎メッセージで mtime を見て読み直すので**保存した次の発言から即反映・再起動不要**（ログには `persona: file … (N chars, mtime …)` と文字数だけ出し、中身は出さない＝ユーザーが Claude に見せずに管理できる）。②は .env なので反映に `sudo systemctl restart discord-music-bot` が要る+ 運用ルール（日本語・短く・雑談では検索しない・調べたら URL 1〜2 個・現在日時 JST・音楽操作は Web アプリへ案内）。`reasoning_effort="low"`（雑談 7〜8 秒、検索込み 15〜20 秒。既定だと雑談に 60 秒かかった）
...
- **統合版 f6bfbbcd（fbf5135f の上に、サブエージェント実装 f909898c をマージ）** — 原因は2つあり、fbf5135f は原因Bのみだった:
  ...
  - **追加 3d9360c8（復帰ロジック強化）**: 再試行を 1s/3s/8s/15s/30s → 以後 60 秒ごと**無期限**に。`probeSession()` が `/api/auth/session` を直接叩き、`200 {}`（サーバーが「セッション無し」）のときだけ本当のログアウトとして IntroPage へ、通信/5xx は待ち続ける（next-auth の `getSession()` は両方 null で区別不能だった）。`online` / `visibilitychange(visible)` で即再確認。「接続を確認しています...」画面に「ログイン画面へ」ボタン（`giveUp`）。検証（オーナーのログイン済み Chromium、`verify-login4.cjs`）: 40 秒通信失敗 → IntroPage に一度も落ちず（recovering 表示）→ online で 1 秒復帰 / `200 {}` 注入 → 2 秒で IntroPage → リロードで復帰。6/6
  - IntroPage のボタン文言は「**Discordで始める**」（テストの正規表現で「Discordでログイン」を探すと誤判定する）
