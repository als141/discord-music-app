...
- **X トレンドメモ（`services/irina_trends.py`）**: grok + x_search/web_search で「今日の X の言い回し・語録の現役/廃れ・派生・今日のネタ」を 1200 字以内にまとめ `knowledge/_auto/x_trends.md` へ。起動 30 秒後（前回から 20 時間超なら）と毎日 `IRINA_TRENDS_HOUR`（既定 18 時 JST。負の値で自動更新オフ）。手動は `refresh_x_trends` ツール（「トレンド更新して」）。**1 回 ≈ $0.34**（x_search が多い）→ 月 $10 前後。ログ `[irina-trends] refreshed`
