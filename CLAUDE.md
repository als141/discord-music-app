...
- **2026-09-22: grok-4.7 に切替 + 明示的な画像生成ツール**: `XAI_MODEL` 既定 **grok-4.7**（同日リリース。`/v1/language-models` で実在確認、text+image 入力、価格は 4.6 と同じ）。画像について実測した事実: ①会話中の `image_generation`（サーバー側ツール）は **JPEG 固定（1408²、$0.059/枚 ≒ grok-imagine-image-2.0 の価格）でモデル指定・透過は不可** ②`client.image.sample()` も JPEG（RGB）で、透過やアスペクト比以外の指定は無い → **`services/irina_images.py` + クライアント側ツール `generate_image`** を追加: モデル指定（既定 `XAI_IMAGE_MODEL`=grok-imagine-image-2.0、`grok-imagine-image-quality`=pro も可）、PNG 出力、`transparent=true` で背景透過（**rembg[cpu] を依存に追加済み**＝AI 背景除去 u2netp。Pi 4 実測: 初回 import+モデル読込 264 秒（コールド）・除去 3.8 秒/枚・メモリ +470MB → `on_ready` で別スレッド先読み `irina_images.warmup()`、ログ `[irina-images] rembg ready in Ns`。`IRINA_REMBG_WARMUP=0` で先読み無効、`IRINA_REMBG_MODEL` でモデル変更。失敗時は四隅の色によるクロマキーにフォールバック。透過時はプロンプトに「単色の白背景」を足す）、アスペクト比、`use_attached_images` で添付を参照画像に。生成物は `ctx["files"]` に集めて返信に添付、1 日の枚数上限は `image_generation` と共有。実測: 透過 PNG 11 秒 $0.04、JPG 16:9 20 秒 $0.04、参照＋quality 7 秒 $0.06。画像モデル一覧は `GET /v1/image-generation-models`（grok-imagine-image $0.02 / -2.0 $0.06 / -quality $0.05 の目安）
