...
async def refresh(client, model: str, *, reason: str = "manual") -> str:
    ...
    async with _lock:
        ...
        prompt = (
            f"今日は {today:%Y年%m月%d日}（日本時間）。日本語圏の X（旧 Twitter）で、いま流行っている言い回し・ネットミーム・スラング・語録の使われ方を "
            "x_search（必要なら web_search も）で調べて、Discord の雑談 bot が『今日の言葉づかい』の参考にできるメモにまとめて。\n"
            "内容:\n"
            "1) 今日〜ここ数日で X で目立つミーム・言い回し・話題（ゲーム・アニメ・時事の流行語も可）。それぞれ 1 行で、実際の使われ方の例つき\n"
            "2) 次の語録リストのうち、今も現役で使われているもの／使われ方が変わったもの／派生形（例: 表記ゆれ、組み合わせ）。実際の投稿での用例を優先し、廃れているものは『今はあまり見ない』と書く\n"
            "3) 今日の X で特に多いネタ（あれば）\n"
            "形式: 見出しと箇条書きだけ。1200 文字以内。指示や評価は書かず、観察した事実だけ。出典 URL は不要。\n\n"
